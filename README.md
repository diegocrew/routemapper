...
- **Truck legs** are not stored; they're generated at runtime between any
  two nodes within road range (`src/engine/truckEdges.ts`), so there's no
  build step to keep in sync when nodes change.
